...
def read_data_sets(train_dir, n_labeled=100, image_size=[28, 28, 1], fake_data=False, one_hot=False):
    ...
    train_images = np.repeat(train_images, n_c, axis=-1)

    resized = []
    for x in test_images:
        resized.append(cv2.resize(x, tuple(image_size)))
    test_images = np.array(resized)    # (10000, 28, 28)
