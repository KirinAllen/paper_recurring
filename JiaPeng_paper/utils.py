...
def avg_pool_7x7(x, output_name="avg"):
    return tf.nn.avg_pool(x, ksize=[1, 7, 7, 1], strides=[1, 7, 7, 1],
...
def un_avg_pool(value, output_shape, name='un_avg_pool'):
    with tf.name_scope(name):
        kernel_shape = (7, 7)
        num_channels = value.get_shape()[-1]
        input_dtype_as_numpy = value.dtype.as_numpy_dtype()
...
def run_transpose_layer(x, layer_spec, output_name="inverse", output_shape=None):
    ...
    elif layer_spec["type"] == "avg":
        z = un_avg_pool(x, output_shape, name=output_name)
    return z